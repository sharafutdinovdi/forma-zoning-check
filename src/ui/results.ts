...
import { boundaryWarning, counts } from "./mini";
...
function updateCard(card: HTMLElement, c: CheckResult): void {
  ...
  card.querySelector(".check-note")!.textContent = missingLimit ? "" : missingGeometry && !/geometry|floor data|footprints/.test(c.note) ? `${c.note}; geometry or floor data unavailable` : c.note;
}
export function renderResults(container: HTMLElement, report: Report, data: SiteData, controls: ParcelControls, _computedAt: number): void {
  if (!container.querySelector(".result-grid")) container.innerHTML = `<div class="section-top"><h2>Results</h2></div><p class="small boundary-summary" role="status" style="border-left:3px solid var(--warn);padding-left:8px" hidden></p><p class="small result-counts"></p><div class="plot-info"></div><p class="small large-plot" hidden>Large plot — make sure the site limit follows the parcel boundary, not the neighbourhood.</p><p class="small no-proposal" hidden>Add a proposal building or include existing buildings in Parcel controls.</p><div class="result-grid">${["far", "coverage", "height", "setbacks"].map((id, i) => `<article class="result-card" data-check="${id}"><div class="card-heading"><h3>${["FAR", "Coverage", "Height", "Setbacks"][i]}</h3><span class="estimate pill"></span></div><span class="status"></span><div class="metric"><span class="metric-value"></span><span class="metric-unit"></span></div><p class="limit"></p><div class="margin-track" aria-hidden="true"><div class="bar-fill"></div></div><p class="margin"></p><p class="check-note"></p></article>`).join("")}</div><div class="other-checks full-only"></div><section class="buildings full-only"><button type="button" class="disclosure" id="buildings-toggle" aria-expanded="false" aria-controls="buildings-body"><span>Buildings</span><span class="building-count"></span></button><div id="buildings-body" class="buildings-body" inert><div class="building-rows"></div></div></section>`;
  ...
  rows.innerHTML = `<p class="small">Colours show each building’s height, floors, setbacks and boundary verdict.</p>` + report.buildings.map(({ building: b, status, checks }) => {
